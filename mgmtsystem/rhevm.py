...
class RHEVMSystem(MgmtSystemAPIBase):
    ...
    * Obj. are not updated after action calls.
    ...
    def connect_direct_lun_to_appliance(self, vm_name, disconnect, creds):
        ...
        Args:
            ...
            host: The hostname/ipaddress of the host
            creds: A dictionary containing {'username': 'username', 'password': 'password'}
        """
        ...
        if not disconnect:
            ...
            while retries < 3:
                ...
                try:
                    ...
                    self.logger.error("Exception caught: %s" % str(e))
                    if retries == 3:
                        self.logger.error("exhausted retries and giving up")
                        raise
                    else:
                        self.logger.info("sleeping for 30s and retrying to connect direct lun")
                        time.sleep(30)
