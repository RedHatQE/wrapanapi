...
class LenovoSystem(WrapanapiAPIBase):
    ...
    _stats_available = {
        ...
        "memory_capacity": lambda self, requester: self.get_server_memory(
            requester.name
        ),
        "num_firmwares": lambda self, requester: len(
            self.get_server_firmwares(requester.name)
        ),
    ...
    _inventory_available = {
        ...
        "ipv4_address": lambda self, requester: self.get_server_ipv4_address(
            requester.name
        ),
        "ipv6_address": lambda self, requester: self.get_server_ipv6_address(
            requester.name
        ),
        "mac_address": lambda self, requester: self.get_server_mac_address(
            requester.name
        ),
        "power_state": lambda self, requester: self.get_server_power_status(
            requester.name
        ),
        "health_state": lambda self, requester: self.get_server_health_state(
            requester.name
        ),
        "manufacturer": lambda self, requester: self.get_server_manufacturer(
            requester.name
        ),
        "model": lambda self, requester: self.get_server_model(requester.name),
        "machine_type": lambda self, requester: self.get_server_machine_type(
            requester.name
        ),
        "serial_number": lambda self, requester: self.get_server_serial_number(
            requester.name
        ),
        "description": lambda self, requester: self.get_server_description(
            requester.name
        ),
        "product_name": lambda self, requester: self.get_server_product_name(
            requester.name
        ),
        "uuid": lambda self, requester: self.get_server_uuid(requester.name),
        "field_replaceable_unit": lambda self, requester: self.get_server_fru(
            requester.name
        ),
    }
    POWERED_ON = 8
    ...
    def __init__(
        self, hostname, username, password, protocol="https", port=None, **kwargs
    ):
    ...
    def list_servers(self):
        ...
        for cabinet in response["cabinetList"]:
            ...
            for chassis in cabinet["chassisList"]:
                ...
                inventory.extend(
                    [node for node in chassis_nodes if node["type"] != "SCU"]
                )
    ...
    def stats(self, *requested_stats, **kwargs):
        ...
        return {
            stat: self._stats_available[stat](self, requester)
    ...
    def inventory(self, *requested_items, **kwargs):
        ...
        return {
            item: self._inventory_available[item](self, requester)
    ...
    def get_network_devices(self, server_name):
        ...
        for addin_card in addin_cards:
            if LenovoSystem.is_network_device(
                addin_card
            ) and not LenovoSystem.is_device_in_list(addin_card, network_devices):
                network_devices.append(addin_card)

        for pci_device in pci_devices:
            if LenovoSystem.is_network_device(
                pci_device
            ) and not LenovoSystem.is_device_in_list(pci_device, network_devices):
    ...
    def get_storage_devices(self, server_name):
        ...
        for addin_card in addin_cards:
            if LenovoSystem.is_storage_device(
                addin_card
            ) and not LenovoSystem.is_device_in_list(addin_card, storage_devices):
                storage_devices.append(addin_card)

        for pci_device in pci_devices:
            if LenovoSystem.is_storage_device(
                pci_device
            ) and not LenovoSystem.is_device_in_list(pci_device, storage_devices):
