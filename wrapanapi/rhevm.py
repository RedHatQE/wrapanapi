...
class RHEVMSystem(WrapanapiAPIBaseVM):
    ...
    * Obj. are not updated after action calls.
    ...
    def list_datastore(self, sd_type=None, **kwargs):
        datastore_list = (
            self.api.system_service().storage_domains_service().list(**kwargs)
        )
        if sd_type:
    ...
    def wait_vm_suspended(self, vm_name, num_sec=720):
        self.logger.info(
            " Waiting for RHEV-M VM %s to change status to SUSPENDED" % vm_name
        )
    ...
    def deploy_template(self, template, *args, **kwargs):
        self.logger.debug(
            " Deploying RHEV template %s to VM %s" % (template, kwargs["vm_name"])
        )
            ...
            with self.steady_wait(30):
                ...
                if self.does_template_exist(temp_template_name):
                    try:
                        ...
                        if (
                            delete
                        ):  # We can only rename to the original name if we delete the vm
                ...
                if create_new_template:
                    ...
                    actual_cluster = (
                        self._get_cluster(cluster) if cluster else vm.cluster
                    )
    ...
    def _get_storage_domain_service(self, name):
        ...
        if not query_result:
            ...
            return self._storage_domains_service.storage_domain_service(
                storage_domain.id
            )
    ...
    def _get_image_service(self, storage_domain_name, image_name):
        for image in self._get_images_service(storage_domain_name).list():
            if image.name == image_name:
                return self._get_images_service(storage_domain_name).image_service(
                    image.id
                )
    ...
    def connect_direct_lun_to_appliance(
        ...
        """
        if not disconnect:
            disk_attachments_service = self._get_vm_service(
                vm_name
            ...
            if not self.does_disk_exist(lun_name):
                ...
                disk_attachment = self._get_disk_attachment_service(
                    vm_name, lun_name
                ).get()
        ...
        # remove it
            ...
            if not self.is_disk_attached_to_vm(vm_name, lun_name):
                ...
                disk_attachment_service = self._get_disk_attachment_service(
                    vm_name, lun_name
                )
