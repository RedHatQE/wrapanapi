...
class ContainersResourceBase(object):
    ...
    def __init__(self, provider, name, namespace):
        ...
        """
        if hasattr(self, "VALID_NAME_PATTERN") and not re.match(
            self.VALID_NAME_PATTERN, name
        ):
            raise InvalidValueException(
                '{0} name "{1}" is invalid. {0} name must '
                'match the regex "{2}"'.format(
                    self.RESOURCE_TYPE, name, self.VALID_NAME_PATTERN
                )
            )
    ...
    def __eq__(self, other):
        return getattr(self, "namespace", None) == getattr(
    ...
    def create(cls, provider, payload):
        ...
        if hasattr(cls, "VALID_NAME_PATTERN") and not re.match(
            cls.VALID_NAME_PATTERN, name
        ):
            raise InvalidValueException(
                '{0} name "{1}" is invalid. {0} name must '
                'match the regex "{2}"'.format(
                    cls.RESOURCE_TYPE, name, cls.VALID_NAME_PATTERN
                )
            )
        ...
        status_code, json_content = api.post(
            cls.RESOURCE_TYPE, payload, namespace=obj.namespace
        )
        # Verifying success
        if status_code not in (200, 201):
            raise RequestFailedException(
                'Failed to create {} "{}". status_code: {}; json_content: {};'.format(
                    cls.RESOURCE_TYPE, obj.name, status_code, json_content
                )
            )
        return obj
    ...
    def get(self, convert=None):
        ...
        status_code, json_content = self.api.get(
            self.RESOURCE_TYPE,
            name=self.name_for_api,
            namespace=self.namespace,
            convert=convert,
        )
        if status_code != 200:
            raise RequestFailedException(
                'GET request of {} "{}" returned status code {}. '
                "json content: {}".format(
                    self.RESOURCE_TYPE, self.name_for_api, status_code, json_content
                )
            )
        return json_content
