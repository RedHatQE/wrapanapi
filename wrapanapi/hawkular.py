...
ServerStatus = namedtuple(
    "ServerStatus", ["address", "version", "state", "product", "host"]
)
Event = namedtuple(
    "event",
    [
...
class Hawkular(WrapanapiAPIBaseVM):
    ...
    def _get_inventory(self, hostname, port, protocol):
        cls = (
            HawkularInventoryInMetrics
            if self._metrics_older("0.26.1.Final")
            else HawkularInventory
        )
        kwargs = dict(
            hostname=hostname,
            port=port,
    ...
    def _metrics_older(self, metrics_version):
        return version.parse(
            self.metric._get("status")["Implementation-Version"]
        ) >= version.parse(metrics_version)
...
class HawkularInventory(HawkularService):
    ...
    def list_server_deployment(self, feed_id=None):
        ...
        resources.extend(
            self.list_resource(feed_id=feed_id, resource_type_id="SubDeployment")
        )
        deployments = []
        if resources:
    ...
    def list_messaging(self, feed_id=None):
        ...
        resources.extend(
            self.list_resource(feed_id=feed_id, resource_type_id="JMS Topic")
        )
        messagings = []
        if resources:
            for resource in resources:
    ...
    def list_server(self, feed_id=None):
        ...
        if resources:
            for resource in resources:
                ...
                servers.append(
                    Server(resource.id, resource.name, resource.path, server_data)
                )
        return servers
    ...
    def list_domain(self, feed_id=None):
        ...
        """
        resources = self.list_resource(
            feed_id=feed_id, resource_type_id="Host Controller"
        )
        domains = []
        if resources:
            for resource in resources:
                ...
                domains.append(
                    Domain(resource.id, resource.name, resource.path, domain_data)
                )
        return domains
    ...
    def list_resource_type(self, feed_id):
        ...
        if entities_j:
            for entity_j in entities_j:
                entities.append(
                    ResourceType(entity_j["id"], entity_j["name"], entity_j["path"])
                )
        return entities
    ...
    def list_operation_definition(self, feed_id, resource_type_id):
        ...
        res_j = self._get(
            "traversal/f;{}/rt;{}/type=ot".format(feed_id, resource_type_id)
        )
        operations = []
        if res_j:
            for res in res_j:
                operations.append(
                    OperationType(res["id"], res["name"], CanonicalPath(res["path"]))
                )
        return operations
    ...
    def list_server_datasource(self, feed_id=None):
        ...
        resources.extend(
            self.list_resource(feed_id=feed_id, resource_type_id="XA Datasource")
        )
        datasources = []
        if resources:
            for resource in resources:
    ...
    def edit_config_data(self, resource_data, **kwargs):
        ...
        r = self._put(
            "entity/f;{}/r;{}/d;configuration".format(
                kwargs["feed_id"], kwargs["resource_id"]
            ),
...
class HawkularInventoryInMetrics(HawkularService):
    ...
    def _list_resource(
        ...
        if resource_type_id:
            data["tags"] = "feed:{},type:r,restypes:.*\\|{}\\|.*".format(
                feed_id, resource_type_id
            )
        else:
        ...
        for entity_j in json.loads(result.content):
            ...
            if entity_value:
                types_index = self._filter_types_index(
                    entity_value["typesIndex"], resource_type_id
                )
    ...
    def get_config_data(self, feed_id, resource_id):
        ...
        if result.status_code == 200:
            ...
            if entity_j:
                try:
                    inventory_j = self._get_data_value(entity_j[0]["data"])[
                        "inventoryStructure"
                    ]
    ...
    def _get_canonical_path(
        self, types_index, data_id, resource_type_path, parent_resource_id
    ):
        ...
        return CanonicalPath(
            "{}/r;{}{}".format(resource_type_path, parent_resource_id, resource_id)
        )
...
class HawkularMetric(HawkularService):
    ...
    def _metric_id_counter_server(feed_id, server_id, metric_enum):
        ...
        if (
            MetricEnumCounter.SVR_TXN_NUMBER_OF_TRANSACTIONS.metric_type
            == metric_enum.metric_type
        ):
    ...
    def list_availability_server(self, feed_id, server_id, **kwargs):
        ...
        Args:
            ...
            kwargs: Refer ``list_availability``
        """
        metric_id = self._metric_id_availability_server(
            feed_id=feed_id, server_id=server_id
        )
        return self.list_availability(metric_id=metric_id, **kwargs)
    ...
    def list_gauge_datasource(
        self, feed_id, server_id, resource_id, metric_enum, **kwargs
    ):
    ...
    def _list_data(self, prefix_id, **kwargs):
        ...
        if not raw and params["bucketDuration"] is None and params["buckets"] is None:
            raise KeyError(
                "Either the 'buckets' or 'bucket_duration' parameter must be used"
            )
        if rate:
    ...
    def add_counter_deployment(
        self, data, feed_id, server_id, resource_id, metric_enum
    ):
...
class HawkularOperation(object):
    ...
    def enable_deployment(
        self, feed_id, server_id, destination_file_name, server_groups=None
    ):
...
class MetricEnumGauge(MetricEnum):
    ...
    DS_POOL_BLOCKING_FAILURE_COUNT = (
...
class MetricEnumCounter(MetricEnum):
    ...
    SVR_MEM_ACCUMULATED_GC_DURATION = (
    ...
    SVR_TXN_NUMBER_OF_RESOURCE_ROLLBACKS = (
        "Transactions Metrics",
