...
class GoogleCloudSystem(WrapanapiAPIBaseVM):
    ...
    def _find_instance_by_name(self, instance_name):
        try:
            ...
            self.logger.error(e)
            self.logger.info(
                "Searching instance {} in all other zones".format(instance_name)
            )
            ...
            for zone in zones.get("items", []):
                ...
                for instance in self._get_zone_instances(zone_name).get("items", []):
                    if instance["name"] == instance_name:
                        return instance
            self.logger.error(
                "Instance {} not found in any of the zones".format(instance_name)
            )
    ...
    def get_image_by_name(self, image_name):
        try:
            image = (
                self._compute.images()
                .get(project=self._project, image=image_name)
                .execute()
            )
            return image
        except Exception:
    ...
    def _nested_operation_wait(self, operation_name, zone=True):
        if not zone:
            ...
            result = (
                ...
                .get(project=self._project, zone=self._zone, operation=operation_name)
                .execute()
            )
        ...
        if result["status"] == "DONE":
            ...
            if "error" in result:
                ...
                self.logger.error(
                    "Detailed information about error {}".format(result["error"])
                )
    ...
    def create_bucket(self, bucket_name):
        ...
        if not self.bucket_exists(bucket_name):
            self._buckets.insert(
                project=self._project, body={"name": "{}".format(bucket_name)}
            ).execute()
            self.logger.info("Bucket {} was created".format(bucket_name))
        else:
            self.logger.info(
                "Bucket {} was not created, exists already".format(bucket_name)
            )
    ...
    def delete_image(self, image_name, timeout=360):
        ...
        operation = (
            self._compute.images()
            .delete(project=self._project, image=image_name)
            .execute()
        )
        ...
        wait_for(
            ...
            delay=0.5,
            num_sec=timeout,
            message=" Deleting image {}".format(image_name),
        )
    ...
    def bucket_exists(self, bucket_name):
        try:
            ...
            if "Invalid bucket name" in error.content:
                self.logger.info(
                    "Incorrect bucket name {} was specified".format(bucket_name)
                )
                return False
            raise error
    ...
    def get_file_from_bucket(self, bucket_name, file_name):
        if self.bucket_exists(bucket_name):
            try:
                data = (
                    self._storage.objects()
                    .get(bucket=bucket_name, object=file_name)
                    .execute()
                )
                return data
            except errors.HttpError as error:
                if "Not Found" in error.content:
                    self.logger.info(
                        "File {} was not found in bucket {}".format(
    ...
    def delete_file_from_bucket(self, bucket_name, file_name):
        if self.bucket_exists(bucket_name):
            try:
                data = (
                    self._storage.objects()
                    .delete(bucket=bucket_name, object=file_name)
                    .execute()
                )
                return data
            except errors.HttpError as error:
    ...
    def upload_file_to_bucket(self, bucket_name, file_path):
        ...
        self.logger.info(
            "Uploading file: {}, to bucket: {}, blob: {}".format(
    ...
    def deploy_template(self, template, **kwargs):
        ...
        machine_type = kwargs.get(
            "machine_type", "zones/{}/machineTypes/n1-standard-1".format(self._zone)
        )
            ...
            "disks": [
                {
                    "boot": True,
                    "autoDelete": True,
                    "initializeParams": {"sourceImage": template_link},
                }
            ],
        ...
        if kwargs.get("ssh_key", None):
            ssh_keys = {"key": "ssh-keys", "value": kwargs.get("ssh_key", None)}
            config["metadata"]["items"].append(ssh_keys)

        operation = self._instances.insert(
            project=self._project, zone=self._zone, body=config
        ).execute()
        wait_for(
            lambda: self._nested_operation_wait(operation["name"]),
            delay=0.5,
            num_sec=timeout,
            message=" Create {}".format(instance_name),
        )
        return True
    ...
    def delete_vm(self, instance_name, timeout=250):
        if not self.does_vm_exist(instance_name):
            self.logger.info(
                "The {} instance is not exists, skipping".format(instance_name)
            )
            return True
        ...
        wait_for(
            ...
            delay=0.5,
            num_sec=timeout,
            message="Delete {}".format(instance_name),
        )
        return True
    ...
    def stop_vm(self, instance_name):
        if self.is_vm_stopped(instance_name) or not self.does_vm_exist(instance_name):
            self.logger.info(
                "The {} instance is already stopped or doesn't exist, skip termination".format(
                    instance_name
                )
            )
            return True
    ...
    def start_vm(self, instance_name):
        ...
        if self.is_vm_running(instance_name) or not self.does_vm_exist(instance_name):
            self.logger.info(
                "The {} instance is already running or doesn't exists, skip starting".format(
                    instance_name
                )
            )
            return True
    ...
    def current_ip_address(self, vm_name):
        ...
        for zone in zones.get("items", []):
            ...
            for vm in self._get_zone_instances(zone_name).get("items", []):
                if vm["name"] == vm_name:
                    access_configs = vm.get("networkInterfaces")[0].get(
                        "accessConfigs"
                    )[0]
    ...
    def wait_vm_running(self, vm_name, num_sec=360):
        self.logger.info(
            "Waiting for instance {} to change status to ACTIVE".format(vm_name)
        )
        wait_for(self.is_vm_running, [vm_name], num_sec=num_sec)

    def wait_vm_stopped(self, vm_name, num_sec=360):
        self.logger.info(
            "Waiting for instance {} to change status to TERMINATED".format(vm_name)
        )
    ...
    def list_network(self):
        ...
        networks = (
            self._compute.networks().list(project=self._project).execute()["items"]
        )
