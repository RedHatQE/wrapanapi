...
class AzureSystem(WrapanapiAPIBaseVM):
    ...
    def __init__(self, **kwargs):
        ...
        self.resource_group = kwargs["provisioning"][
            "resource_group"
    ...
    def start_vm(self, vm_name, resource_group=None):
        ...
        operation = col.start(
            resource_group_name=resource_group or self.resource_group, vm_name=vm_name
        )
        operation.wait()
    ...
    def capture_vm(self, vm_name, container, image_name, overwrite_vhds=True, **kwargs):
        ...
        operation = col.capture(
            resource_group_name=resource_group, vm_name=vm_name, parameters=params
        )
        operation.wait()
        return operation.status()
    ...
    def is_vm_running(self, vm_name, resource_group=None):
        if self.vm_status(vm_name, resource_group) == self.STATE_RUNNING:
            self.logger.info(
                'According to Azure, the VM "{}" is running'.format(vm_name)
            )
            return True
        else:
            return False
    ...
    def is_vm_stopped(self, vm_name, resource_group=None):
        if self.vm_status(vm_name, resource_group) == self.STATE_STOPPED:
            self.logger.info(
                'According to Azure, the VM "{}" is stopped'.format(vm_name)
            )
            return True
        else:
            return False
    ...
    def is_vm_starting(self, vm_name, resource_group=None):
        if self.vm_status(vm_name, resource_group) == self.STATE_STARTING:
            self.logger.info(
                'According to Azure, the VM "{}" is starting'.format(vm_name)
            )
            return True
        else:
            return False
    ...
    def is_vm_suspended(self, vm_name, resource_group=None):
        if self.vm_status(vm_name, resource_group) == self.STATE_SUSPEND:
            self.logger.info(
                'According to Azure, the VM "{}" is suspended'.format(vm_name)
            )
            return True
        else:
            return False
    ...
    def vm_creation_time(self, vm_name, resource_group=None):
        ...
        self.logger.info(
            "Attempting to Retrieve Azure VM Modification Time {}".format(vm_name)
        )
        vm = self.vms_collection.get(
            resource_group_name=resource_group or self.resource_group,
            vm_name=vm_name,
    ...
    def get_network_interface(self, vm_name, resource_group=None):
        ...
        self.logger.info(
            "Attempting to Retrieve Azure VM Network Interface %s", vm_name
        )
        vm = self.vms_collection.get(
            resource_group_name=resource_group or self.resource_group, vm_name=vm_name
        )
        ...
        *_id - is a valid Azure resource id
        ...
        for ip_config in if_obj.ip_configurations:
            if ip_config.primary is True:
                ...
                break
        ip_config_obj = self.network_client.network_interface_ip_configurations.get(
            resource_group, if_name, ip_config_name
        )
        ...
        try:
            ...
            if not public_ip.ip_address:
                ...
                self.logger.error(
                    "Couldn't get Public IP of {}. public_ip_allocation_method - {}. "
                    "Please check VM status".format(
                        vm_name, public_ip.public_ip_allocation_method
                    )
                )
                return None
            ...
            self.logger.error(
                "VM {} doesn't have public IP on {}:{}".format(
                    vm_name, if_name, ip_config_name
                )
            )
            return None
    ...
    def get_ip_address(self, vm_name, resource_group=None, **kwargs):
        current_ip_address = self.current_ip_address(
            vm_name, resource_group or self.resource_group
        )
    ...
    def list_region(self, subscription=None):
        ...
        return [
            ...
            for region in self.subscription_client.subscriptions.list_locations(
                subscription
            )
        ]
    ...
    def list_storage_accounts_by_resource_group(self, resource_group):
        ...
        return [
            s.name
            for s in self.storage_client.storage_accounts.list_by_resource_group(
                resource_group
            )
        ]
    ...
    def list_free_pip(self, pip_template, resource_group=None):
        ...
        """
        resource_group = resource_group or self.resource_group
        ips = self.network_client.public_ip_addresses.list(
            resource_group_name=resource_group
        )
        return [
            ip.name for ip in ips if not ip.ip_configuration and pip_template in ip.name
        ]
    ...
    def list_free_nics(self, nic_template, resource_group=None):
        ...
        """
        resource_group = resource_group or self.resource_group
        ips = self.network_client.network_interfaces.list(
            resource_group_name=resource_group
        )
        return [
            ip.name for ip in ips if not ip.virtual_machine and nic_template in ip.name
        ]
    ...
    def list_network(self):
        self.logger.info(
            "Attempting to list Azure Virtual Private Networks in '%s'", self.region
        )
    ...
    def remove_nics_by_search(self, nic_template, resource_group=None):
        ...
        """
        self.logger.info(
            'Attempting to List NICs with "%s" name template', nic_template
        )
        results = []
        ...
        for nic in nic_list:
            try:
                ...
                self.logger.error(
                    "{} nic can't be removed - {}".format(nic, e.error.error)
                )
    ...
    def remove_pips_by_search(self, pip_template, resource_group=None):
        ...
        """
        self.logger.info(
            'Attempting to List Public IPs with "%s" name template', pip_template
        )
        results = []
    ...
    def list_blob_images(self, container):
        return [
            blob.name
            for blob in self.container_client.list_blobs(container_name=container)
    ...
    def delete_stack(self, stack_name, resource_group=None):
        ...
        """
        self.logger.info(
            "Removes a Deployment Stack resource created with Orchestration"
        )
        deps = self.resource_client.deployments
    ...
    def delete_stack_by_date(self, days_old, resource_group=None):
        ...
        self.logger.info(
            "Removes a Deployment Stack resource older than %s days", days_old
        )
        results = []
    ...
    def deploy_template(self, template, vm_name=None, **vm_settings):
        ...
        vm_parameters = {
            "location": location,
            "hardware_profile": {"vm_size": getattr(VirtualMachineSizeTypes, vm_size)},
            "storage_profile": {
                "os_disk": {
                    "os_type": "Linux",
                    "name": vm_name,
    ...
    def remove_container_blob(
        self, container_client, container, blob, remove_snapshots=True
    ):
        self.logger.info(
            "Removing Blob '%s' from containter '%s'", blob.name, container.name
        )
        try:
            ...
            if "SnapshotsPresent" in str(e) and remove_snapshots:
                self.logger.warn(
                    "Blob '%s' has snapshots present, removing them", blob.name
                )
                container_client.delete_blob(
    ...
    def remove_unused_blobs(self, resource_group=None):
        ...
        for storage_account in self.list_storage_accounts_by_resource_group(
            resource_group
        ):
        ...
        for disk in self.compute_client.disks.list_by_resource_group(resource_group):
            if disk.name.startswith("test") and disk.managed_by is None:
                ...
                removed_disks.append(
                    {"resource_group": resource_group, "disk": disk.name}
                )
