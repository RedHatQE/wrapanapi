...
def mocked_image_data():
    ...
    for i in range(fauxfactory.gen_integer(2, 20)):
        ...
        out[1]["items"][-1]["dockerImageReference"] = dockerImageReference.format(
...
def wait_for_existence(resource, exist=True, timeout="1M"):
    ...
    return wait_for(
        ...
        message="Waiting for {} {} to {}exist...".format(
            resource.RESOURCE_TYPE, resource.name, ("" if exist else "not ")
        ),
        delay=5,
        timeout=timeout,
    )
...
def base__test_label_create(resource, label_key, label_value):
    if MOCKED:
        resource.provider.api.patch.return_value = (
            resource.provider.o_api.patch.return_value
        ) = [
            201,
            {},
        ]
        resource.provider.api.get.return_value = (
            resource.provider.o_api.get.return_value
        ) = [
            200,
    ...
    assert wait_for(
        lambda: label_key in resource.list_labels(),
        message="Waiting for label {} of {} {} to exist...".format(
            label_key, type(resource).__name__, resource.name
        ),
        delay=5,
        timeout="1M",
    ).out
...
class TestProject(object):
    def test_list(self, provider):
        if MOCKED:
            provider.o_api.get.return_value = [
                200,
                {
                    "items": [
...
class TestImage(object):
    def test_list(self, provider):
        ...
        assert all(
            [isinstance(inst, Image) for inst in provider.list_image_openshift()]
        )
...
class TestPod(object):
    ...
    def test_properties(self, gen_pod):
        # Just test that there are no errors when we try to get properties
        if MOCKED:
            gen_pod.provider.api.get.return_value = [
                200,
                {"spec": {"restartPolicy": "Always", "dnsPolicy": "Sometimes"}},
            ]
...
class TestService(object):
    ...
    def test_properties(self, gen_service):
        # Just test that there are no errors when we try to get properties
        if MOCKED:
            gen_service.provider.api.get.return_value = [
                200,
                {"spec": {"sessionAffinity": "ClientIP", "clusterIP": "127.0.0.1"}},
            ]
    ...
    def test_create(self, provider, gen_route, gen_service):
        ...
        payload = {
            ...
            "spec": {
                ...
                "to": {"Kind": service.kind(), "name": service.name},
            },
        }
...
class TestReplicator(object):
    ...
    def test_properties(self, gen_replicator):
        ...
        if MOCKED:
            ...
            gen_replicator.provider.api.get.return_value = [
                200,
...
class TestDeploymentConfig(object):
    def test_list(self, provider):
        if MOCKED:
            provider.api.post.return_value = provider.o_api.get.return_value = [
                200,
                {
                    "items": [
                        {
        ...
        assert all(
            [
                isinstance(inst, DeploymentConfig)
    ...
    def test_dc_create(self, provider, gen_dc):
        payload = {
            "metadata": {"name": gen_dc.name, "namespace": gen_dc.namespace},
            "spec": {
                "replicas": 1,
                "test": False,
                ...
                "template": {
                    "metadata": {"labels": {"run": gen_dc.name}},
                    "spec": {
                        "containers": [
                            {
...
class TestImageRegistry(object):
    def test_list(self, provider):
        ...
        assert all(
            [isinstance(inst, ImageRegistry) for inst in provider.list_image_registry()]
        )
        assert all(
            [
                isinstance(inst, ImageRegistry)
    ...
    def test_import_image(self, provider, gen_image_registry):
        if MOCKED:
            ...
            provider.o_api.post.return_value = provider.o_api.get.return_value = [
                200,
                {
                    "status": {
                        "dockerImageRepository": docker_image_reference,
                        "images": [
                            {"image": {"dockerImageReference": docker_image_reference}}
                        ],
                    }
                },
            ]
...
class TestVolume(object):
    def test_create(self, provider, gen_volume):
        payload = {
            ...
            "spec": {
                ...
                "nfs": {"path": "/tmp", "server": "12.34.56.78"},
            },
    ...
    def test_properties(self, gen_volume):
        # Just test that there are no errors when we try to get properties
        if MOCKED:
            gen_volume.api.get.return_value = [
                200,
                {
                    "spec": {
